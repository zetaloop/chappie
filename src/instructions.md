...
Use transfer with paths and files to copy ChatGPT files into Pi. Omit files to expose existing Pi paths or chappi:// image references as MCP resources. Relative paths use the Pi working directory, existing targets require overwrite: true, and resource materialization may require host confirmation.

...
Use history to read the current Pi branch with original entry IDs and timestamps, including chat activity records. It defaults to the last 20 entries; before pages backward and after pages forward. History is a record of past work, separate from new Pi input. It leaves a reading notice and returns its content only to the caller.
